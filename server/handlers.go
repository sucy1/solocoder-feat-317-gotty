...
func (server *Server) processWSConn(ctx context.Context, conn *websocket.Conn) error {
	...
	if server.options.Record {
		recordFile := server.options.RecordFile
		if recordFile == "" {
			recordFile = webtty.DefaultRecordFilePath()
		} else {
			recordFile = homedir.Expand(recordFile)
		}
		recorder, err := webtty.NewRecorder(recordFile)
		if err != nil {
			return errors.Wrapf(err, "failed to create recorder")
		}
		log.Printf("Recording session to: %s", recordFile)
		opts = append(opts, webtty.WithRecorder(recorder))
	}

	if server.options.Play != "" {
		playFile := homedir.Expand(server.options.Play)
		player, err := webtty.NewPlayer(playFile)
		if err != nil {
			return errors.Wrapf(err, "failed to create player")
		}
		log.Printf("Playing back session from: %s", playFile)
		opts = append(opts, webtty.WithPlayer(player))
	}
...
func (server *Server) handleConfig(w http.ResponseWriter, r *http.Request) {
	...
	w.Write([]byte("var gotty_term = '" + server.options.Term + "'; var gotty_play_mode = " + playMode + ";"))
}
